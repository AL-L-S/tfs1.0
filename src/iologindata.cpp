...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if ((result = db->storeQuery(query.str()))) {
		...
		for (ItemMap::reverse_iterator it = itemMap.rbegin(); it != itemMap.rend(); ++it) {
			...
			int32_t pid = pair.second;
			if (pid >= 0 && pid < 100) {
				DepotChest* depotChest = player->getDepotLockerChest(pid, true);
				if (depotChest) {
					depotChest->internalAddThing(item);
				}

				/*DepotLocker* depotChest2 = player->getDepotLocker(pid);
				if (depotChest2) {
					depotChest2->internalAddThing(item);
				}*/
			} else {
				ItemMap::const_iterator it2 = itemMap.find(pid);
				if (it2 == itemMap.end()) {
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	if (player->lastDepotId != -1) {
		...
		for (const auto& it : player->depotLockerMap) {
			DepotLocker* depotChest = it.second;
