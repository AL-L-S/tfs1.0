...
void LuaScriptInterface::registerFunctions()
{
	...
	//getDepotLockerId(uid)
	lua_register(m_luaState, "getDepotLockerId", LuaScriptInterface::luagetDepotLockerId);
	...
	registerEnum(MSG_STATUS_CONSOLE_BLUE)
	registerEnum(MSG_STATUS_CONSOLE_RED)
	registerEnum(MSG_STATUS_DEFAULT)
	registerEnum(MSG_STATUS_WARNING)
	registerEnum(MSG_EVENT_ADVANCE)
	registerEnum(MSG_STATUS_SMALL)
	registerEnum(MSG_INFO_DESCR)
	registerEnum(MSG_DAMAGE_DEALT)
	registerEnum(MSG_DAMAGE_RECEIVED)
	registerEnum(MSG_HEALED)
	registerEnum(MSG_EXPERIENCE)
	registerEnum(MSG_DAMAGE_OTHERS)
	registerEnum(MSG_HEALED_OTHERS)
	registerEnum(MSG_EXPERIENCE_OTHERS)
	registerEnum(MSG_EVENT_DEFAULT)
	registerEnum(MSG_EVENT_ORANGE)
	registerEnum(MSG_STATUS_CONSOLE_ORANGE)
	...
	registerEnum(SPEAK_SAY)
	registerEnum(SPEAK_WHISPER)
	registerEnum(SPEAK_YELL)
	registerEnum(SPEAK_PRIVATE_FROM)
	registerEnum(SPEAK_PRIVATE_TO)
	registerEnum(SPEAK_CHANNEL_Y)
	registerEnum(SPEAK_CHANNEL_O)
	registerEnum(SPEAK_PRIVATE_NP)
	registerEnum(SPEAK_PRIVATE_PN)
	registerEnum(SPEAK_BROADCAST)
	registerEnum(SPEAK_CHANNEL_R1)
	registerEnum(SPEAK_PRIVATE_RED_FROM)
	registerEnum(SPEAK_PRIVATE_RED_TO)
	registerEnum(SPEAK_MONSTER_SAY)
	registerEnum(SPEAK_MONSTER_YELL)
	registerEnum(SPEAK_CHANNEL_R2)
	...
	registerMethod("Player", "getDepotLockerChest", LuaScriptInterface::luaPlayergetDepotLockerChest);
...
int32_t LuaScriptInterface::luagetDepotLockerId(lua_State* L)
{
	//getDepotLockerId(uid)
	uint32_t uid = getNumber<uint32_t>(L, -1);
	...
	if (container) {
		...
		if (depotLocker) {
			lua_pushnumber(L, depotLocker->getDepotLockerId());
		} else {
...
int32_t LuaScriptInterface::luaPlayergetDepotLockerChest(lua_State* L)
{
	// player:getDepotLockerChest(depotId[, autoCreate = false])
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	DepotChest* depotChest = player->getDepotLockerChest(depotId, autoCreate);
	if (depotChest) {
...
int32_t LuaScriptInterface::luaPlayerSendTextMessage(lua_State* L)
{
	...
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->sendTextMessage(message.type, message.text);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int32_t LuaScriptInterface::luaPlayerSendPrivateMessage(lua_State* L)
{
	...
	const std::string& text = getString(L, 3);
	SpeakClasses type = getNumber<SpeakClasses>(L, 4, SPEAK_PRIVATE_FROM);
