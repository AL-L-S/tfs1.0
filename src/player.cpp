...
DepotChest* Player::getDepotLockerChest(uint32_t depotId, bool autoCreate)
{
...
DepotLocker* Player::getDepotLocker(uint32_t depotId)
{
	...
	depotLocker->internalAddThing(getDepotLockerChest(depotId, true));
...
void Player::removeExperience(uint64_t exp, bool sendText/* = false*/)
{
	...
	if (sendText) {
		...
		sendExperienceMessage(MSG_EXPERIENCE, ss.str(), targetPos, lostExp, TEXTCOLOR_RED);
		...
		for (Creature* spectator : list) {
			Player* tmpPlayer = spectator->getPlayer();
			if (tmpPlayer != this) {
				tmpPlayer->sendExperienceMessage(MSG_EXPERIENCE_OTHERS, strExp, targetPos, lostExp, TEXTCOLOR_RED);
			}
		}
	}
